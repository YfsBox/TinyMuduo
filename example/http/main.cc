...
void onRequest(const TinyHttp::HttpRequest& req, TinyHttp::HttpReponse* resp) {
    ...
    if (req.getUrl() == "/") {
        resp->setHttpStatusCode(TinyHttp::HttpReponse::Ok200);
        resp->setStatusMessage("OK");
        resp->setContentType("text/html");
        resp->addHeaderKv("Server", "Muduo");
        std::string now = TinyMuduo::TimeStamp::getNowTimeStamp().getTimeFormatString();
        ...
        resp->setHttpStatusCode(TinyHttp::HttpReponse::NotFound404);
